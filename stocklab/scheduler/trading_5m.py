...
def check_buy_order(code):
    ...
    for order in order_list:
        ...
        if order_cnt == check_result:
            mongo.update_item({"매수주문.주문번호":order_no},{"$set":{"매수완료":check_result[0],"status":"buy_completed"}},"stocklab_demo","order")
...
def check_sell_order(code):
    ...
    for order in sell_order_list:
        time.sleep(1)
        code = order["code"]
        order_no = order["매도문"]["주문번호"]
        order_cnt = order["매도주문"]["실물주문수량"]
        check_result = ebest_demo.order_check(order_no)
        print("check sell order result", check_result)
        result_cnt = check_result["체결수량"]
        if order_cnt == check_result:
            mongo.update_item({"매도주문.주문번호": order_no}, {"$set": {"매도완료": check_result[0], "status": "sell_completed"}},
                              "stocklab_demo", "order")
...
def trading_scenario(code_list):
    for code in code_list:
        ...
        if buy_order_cnt == 0:
            ...
            order_doc = order[0]
            mongo.insert_item({"매수주문":order_doc,"code":code,"status":"buy_ordered"},"stock_lab","order")
...
if __name__ == '__main__':
    ...
    print("today:",today)
    scheduler.add_job(func=run_process_trading_scenario,trigger="cron",minute=5,id="demo",kwargs={"code_list":code_list})
    scheduler.start()
    while True:
